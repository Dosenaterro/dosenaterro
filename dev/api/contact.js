  ...
  const row = (icon, label, value) => `
    <tr>
      <td style="padding:0 0 16px 0;vertical-align:top;width:36px">
        <div style="width:36px;height:36px;background:linear-gradient(135deg,#06A77D,#005377);border-radius:10px;text-align:center;line-height:36px;font-size:16px">${icon}</div>
      </td>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 20px 60px rgba(5,47,95,0.12)">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#052F5F 0%,#005377 60%,#06A77D 100%);padding:40px 40px 32px">
            ...
            <h1 style="margin:0 0 8px;font-size:26px;font-weight:700;color:#fff;line-height:1.2">🔔 Nouveau message de contact</h1>
            <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.75)">Reçu le ${now}</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:0 40px">
            <div style="background:linear-gradient(135deg,rgba(6,167,125,0.08),rgba(0,83,119,0.05));border:1px solid rgba(6,167,125,0.2);border-radius:0 0 16px 16px;padding:16px 20px">
              <div style="display:inline-block;width:44px;height:44px;background:linear-gradient(135deg,#06A77D,#005377);border-radius:50%;text-align:center;line-height:44px;font-size:18px;font-weight:700;color:#fff;vertical-align:middle">${esc(name).charAt(0).toUpperCase()}</div>
        ...
        <tr>
          <td style="padding:0 40px 32px">
            <a href="mailto:${esc(email)}?subject=Re%3A%20Votre%20message&body=Bonjour%20${encodeURIComponent(name)}%2C%0A%0A"
               style="display:inline-block;background:linear-gradient(135deg,#06A77D,#005377);color:#fff;font-size:15px;font-weight:600;text-decoration:none;padding:14px 28px;border-radius:12px;box-shadow:0 8px 24px rgba(6,167,125,0.35)">
              ↩ Répondre à ${esc(name)}
            </a>
          </td>
        </tr>
        ...
        <!-- DIVIDER -->
        <tr><td style="padding:0 40px"><div style="height:1px;background:rgba(5,47,95,0.1);margin-bottom:24px"></div></td></tr>

        <!-- FOOTER -->
        <tr>
          <td style="padding:0 40px 40px">
            <p style="margin:0 0 4px;font-size:13px;color:rgba(5,47,95,0.5)">Ce message a été envoyé depuis le formulaire de contact de votre portfolio.</p>
            <p style="margin:0;font-size:12px;color:rgba(5,47,95,0.35)">Email généré automatiquement — répondez directement à l'expéditeur via le bouton ci-dessus.</p>
          </td>
        </tr>
